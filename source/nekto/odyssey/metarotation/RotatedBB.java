...
public class RotatedBB extends AxisAlignedBB 
{
	...
	private RotatedBB(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, double rot) {
		super(minX, minY, minZ, maxX, maxY, maxZ);
		rotation = rot;
	}

	public static RotatedBB getBoundingBox(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, double rot)
	{
		return new RotatedBB(minX, minY, minZ, maxX, maxY, maxZ, rot);
	}

	public static RotatedBB getBoundingBoxFromPool(double minX, double minY, double minZ, double maxX, double maxY, double maxZ)
	{
		return getBoundingBoxFromPool(minX, minY, minZ, maxX, maxY, maxZ, 0);
	}

	public static RotatedBB getBoundingBoxFromPool(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, double rot)
	{
		return new RotatedBB(minX, minY, minZ, maxX, maxY, maxZ, rot);
	}
	...
	public static RotatedBB getBoundingBoxFromPool(AxisAlignedBB box)
	{
		...
		{
			return getBoundingBoxFromPool(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, ((RotatedBB) box).rotation);
		}
		else
		{
			return getBoundingBoxFromPool(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, 0);
		}
	}
	
	public static RotatedBB getBoundingBoxFromPool(AxisAlignedBB box, double rot)
	{
		return getBoundingBoxFromPool(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, rot);
	}
	...
	@Override
	public RotatedBB setBounds(double d, double d1, double d2, double d3, double d4, double d5)
	{
		minX = d;
		minY = d1;
		minZ = d2;
	...
	public RotatedBB expand(double d, double d1, double d2)
	{
		...
		return getBoundingBoxFromPool(minX - d3, minY - d1, minZ - d3, maxX + d3, maxY + d1, maxZ + d3, rotation);
	}
	...
	protected boolean isVecInYZ(Vec3 vec3d)
    {
        ...
        } else
        {
            ...
            return vec3d.yCoord >= minY && vec3d.yCoord <= maxY && vec3d.zCoord >= box.minZ && vec3d.zCoord <= box.maxZ;
        }
    }
    ...
    protected boolean isVecInXY(Vec3 vec3d)
    {
        ...
        {
        	AxisAlignedBB box = getExpandedAABB();
            return vec3d.xCoord >= box.minX && vec3d.xCoord <= box.maxX && vec3d.yCoord >= minY && vec3d.yCoord <= maxY;
        }
    }
	...
	{
		return getBoundingBoxFromPool(minX + d, minY + d1, minZ + d2, maxX + d, maxY + d1, maxZ + d2, rotation);
	}
	...
	private Vec3 calculateXZOffset(AxisAlignedBB axisalignedbb, Vec3 vecOffset)
	{
		...
		// Calculate corners.
		Vec3 vecA = rotateVec3AroundCenter(Vec3.createVectorHelper(box1.minX, 0, box1.minZ), box1.rotation);
		Vec3 vecB = rotateVec3AroundCenter(Vec3.createVectorHelper(box1.minX, 0, box1.maxZ), box1.rotation);
		Vec3 vecC = rotateVec3AroundCenter(Vec3.createVectorHelper(box1.maxX, 0, box1.maxZ), box1.rotation);
		Vec3 vecD = rotateVec3AroundCenter(Vec3.createVectorHelper(box1.maxX, 0, box1.minZ), box1.rotation);
		Vec3 vecK = rotateVec3AroundCenter(Vec3.createVectorHelper(box2.minX, 0, box2.minZ), box2.rotation);
		Vec3 vecL = rotateVec3AroundCenter(Vec3.createVectorHelper(box2.minX, 0, box2.maxZ), box2.rotation);
		Vec3 vecM = rotateVec3AroundCenter(Vec3.createVectorHelper(box2.maxX, 0, box2.maxZ), box2.rotation);
		Vec3 vecN = rotateVec3AroundCenter(Vec3.createVectorHelper(box2.maxX, 0, box2.minZ), box2.rotation);
		...
		for (Vec3 intersection : intersections)
		{
			...
			{
				vecOffset.xCoord = Math.min(vecOffset.xCoord, Math.max(intersection.xCoord, 0));
			}
			if (vecOffset.xCoord < 0)
			{
				vecOffset.xCoord = Math.max(vecOffset.xCoord, Math.min(intersection.xCoord, 0));
			}
			if (vecOffset.zCoord > 0)
			{
				vecOffset.zCoord = Math.min(vecOffset.zCoord, Math.max(intersection.zCoord, 0));
			}
			if (vecOffset.zCoord < 0)
			{
				vecOffset.zCoord = Math.max(vecOffset.zCoord, Math.min(intersection.zCoord, 0));
			}
		}
	...
	public boolean intersectsWith(AxisAlignedBB axisalignedbb) {	
		...
		Vec3 a = rotateVec3AroundCenter(Vec3.createVectorHelper(minX, 0, minZ), rotation);
		Vec3 b = rotateVec3AroundCenter(Vec3.createVectorHelper(minX, 0, maxZ), rotation);
		Vec3 c = rotateVec3AroundCenter(Vec3.createVectorHelper(maxX, 0, maxZ), rotation);
		Vec3 d = rotateVec3AroundCenter(Vec3.createVectorHelper(maxX, 0, minZ), rotation);
		
		// Intersect segments AB, BC, CD and DA with box1.
		if (box2.calculateIntercept(a, b) != null || box2.calculateIntercept(b, c) != null || box2.calculateIntercept(c, d) != null || box2.calculateIntercept(d, a) != null)
		{
			return true;
		}

		//Either boxes don't intersect, or one is completely inside another.
		if (box1.isVecInside(Vec3.createVectorHelper((box2.minX + box2.maxX) / 2, 0, (box2.minZ + box2.maxZ) / 2))) {
			return true;
		}
		if (box2.isVecInside(Vec3.createVectorHelper((box1.minX + box1.maxX) / 2, 0, (box1.minZ + box1.maxZ) / 2))) {
			return true;
		}
		return false;
	}
	...
	{
		return getBoundingBoxFromPool(minX, minY, minZ, maxX, maxY, maxZ, rotation);
	}
	...
	{
		return (new StringBuilder()).append("box[").append(minX).append(", ").append(minY).append(", ").append(minZ).append(" -> ").append(maxX).append(", ").append(maxY).append(", ").append(maxZ).append(" -> ").append(rotation).append("]").toString();
	}
	...
	{
		Vec3 center = Vec3.createVectorHelper((minX + maxX) / 2, (minY + maxY) / 2, (minZ + maxZ) / 2);
		return rotateVec3AroundVec3(vec, center, rot);
	}
	
	public RotatedBB rotateAroundVec3(Vec3 vec, double rot)
	{
		Vec3 oldCenter = Vec3.createVectorHelper((minX + maxX) / 2, (minY + maxY) / 2, (minZ + maxZ) / 2);
	...
	{
		Vec3 a = rotateVec3AroundCenter(Vec3.createVectorHelper(minX, minY, minZ), rotation);
		Vec3 b = rotateVec3AroundCenter(Vec3.createVectorHelper(minX, minY, maxZ), rotation);
		Vec3 c = rotateVec3AroundCenter(Vec3.createVectorHelper(maxX, minY, maxZ), rotation);
		Vec3 d = rotateVec3AroundCenter(Vec3.createVectorHelper(maxX, minY, minZ), rotation);
		
		double expandedMinX = Math.min(a.xCoord, Math.min(b.xCoord, Math.min(c.xCoord, d.xCoord)));
		double expandedMaxX = Math.max(a.xCoord, Math.max(b.xCoord, Math.max(c.xCoord, d.xCoord)));
		double expandedMinZ = Math.min(a.zCoord, Math.min(b.zCoord, Math.min(c.zCoord, d.zCoord)));
		double expandedMaxZ = Math.max(a.zCoord, Math.max(b.zCoord, Math.max(c.zCoord, d.zCoord)));
		
		return super.getBoundingBox(expandedMinX, minY, expandedMinZ, expandedMaxX, maxY, expandedMaxZ);
	}
