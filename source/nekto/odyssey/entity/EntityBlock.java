...
public class EntityBlock extends Entity
{
	...
	public static Hashtable<Integer, Class> entityClassLookup = new Hashtable<Integer, Class>(
			4);
	@SuppressWarnings("rawtypes")
	public static Hashtable<Class, Integer> entityClassReverseLookup = new Hashtable<Class, Integer>(
			4);
	...
	public void rejoinWorld(boolean notify)
	{
		...
		BlockMetaPair newBlock = MetaRotation.rotateMeta(blockId, meta,
				currentDirection);

		if (notify)
		{
			worldObj.setBlockMetadataWithNotify((int) Math.round(posX - 0.5),
					(int) Math.round(posY - 0.5), (int) Math.round(posZ - 0.5),
					newBlock.id, newBlock.meta);
		} else
		{
			worldObj.setBlock((int) Math.round(posX - 0.5),
					(int) Math.round(posY - 0.5), (int) Math.round(posZ - 0.5),
					newBlock.id, newBlock.meta, 2);
		}

		worldObj.markBlockForUpdate((int) Math.round(posX - 0.5),
				(int) Math.round(posY - 0.5), (int) Math.round(posZ - 0.5));
	...
	public void updatePositionFromConsole()
	{
		...
		posX = getGrid().ref.posX
				+ gridX
				* MathHelper
						.cos((float) (-getGrid().ref.rotationYaw / 180 * (Math.PI)))
				- gridZ
				* MathHelper
						.sin((float) (-getGrid().ref.rotationYaw / 180 * (Math.PI)));
		posY = getGrid().ref.posY + gridY;
		posZ = getGrid().ref.posZ
				+ gridX
				* MathHelper
						.sin((float) (-getGrid().ref.rotationYaw / 180 * (Math.PI)))
				+ gridZ
		...
		if (bb != null)
		{
			// bb.contract((bb.maxX - bb.minX)/2*Math.cos(rotationYaw%90), 0,
			// (bb.maxZ - bb.minZ)/2*Math.cos(rotationYaw%90));
		...
		} else
		{
			...
			if (block != null)
			{
				...
				boundingBox.setBounds(posX + block.getBlockBoundsMinX() - 0.5,
			...
			{
				boundingBox.setBounds(posX - 0.5, posY - 0.5, posZ - 0.5,
	...
	public AxisAlignedBB getBoundingBox()
	{
		...
		{
			block.setBlockBoundsBasedOnState(getGrid(), gridX, gridY, gridZ);
			AxisAlignedBB alignedBox = block.getCollisionBoundingBoxFromPool(
					getGrid(), gridX, gridY, gridZ);

			if (alignedBox != null)
			{
				RotatedBB rotatedBox = RotatedBB.getBoundingBoxFromPool(
						alignedBox, 0);

				rotatedBox.offset(-0.5D, -0.5D, -0.5D);
				rotatedBox.rotateAroundVec3(
						Vec3.createVectorHelper(0.0D, 0.0D, 0.0D), rotationYaw);
				rotatedBox.offset(getGrid().ref.posX, getGrid().ref.posY,
						getGrid().ref.posZ);
	...
	public boolean interactFirst(EntityPlayer entityplayer)
	{
		...
		// TODO: Fix
		if (Block.blocksList[blockId].onBlockActivated(grid, gridX, gridY,
				gridZ, entityplayer, meta, 0, 0, 0))
		{
			return true;
		}
		...
		if (stack != null && Item.itemsList[stack.itemID] != null)
		{
			...
			Vec3 vec3d2 = vec3d.addVector(vec3d1.xCoord * 10,
					vec3d1.yCoord * 10, vec3d1.zCoord * 10);

			RotatedBB box = RotatedBB.getBoundingBoxFromPool(boundingBox,
					rotationYaw);
			if (box != null)
			{
				MovingObjectPosition mop = box
				...
				if (mop != null)
				{
					...
					// TODO: Fix
					if (item.onItemUse(stack, entityplayer, getGrid(), gridX,
							gridY, gridZ, side, 0, 0, 0))
					{
	...
	{
		int blockGridEntityClassID = EntityBlock.entityClassReverseLookup
				.get(this.getClass());
	...
	public static EntityBlock createChildBlockFromNBT(NBTTagCompound childData)
	{
		...
		{
			System.out
					.printf("Unhandled InstantiationException for BlockGridEntity ID %d\n",
							entityID);
			return null;
		} catch (IllegalAccessException e)
		{
			System.out
					.printf("Unhandled IllegalAccessException for BlockGridEntity ID %d\n",
							entityID);
			return null;
		} catch (IllegalArgumentException e)
		{
			System.out
					.printf("Unhandled IllegalArgumentException for BlockGridEntity ID %d\n",
							entityID);
			return null;
		} catch (SecurityException e)
		{
			System.out.printf(
					"Unhandled SecurityException for BlockGridEntity ID %d\n",
					entityID);
			return null;
		} catch (InvocationTargetException e)
		{
			System.out
					.printf("Unhandled InvocationTargetException for BlockGridEntity ID %d\n",
							entityID);
			return null;
		} catch (NoSuchMethodException e)
		{
			System.out
					.printf("Unhandled NoSuchMethodException for BlockGridEntity ID %d\n",
							entityID);
			return null;
		}
	}
	...
	{
		return (test.gridX == this.gridX && test.gridY == this.gridY && test.gridZ == this.gridZ);
	}
	 ...
	 * 
	 * public EntityBlock(World par1World, int x, int y, int z, int id, int
	 * meta) { super(par1World);
	 * 
	 * setSize(1.0F, 1.0F); entityCollisionReduction = 1; this.yOffset =
	 * (this.height / 2); this.setLocationAndAngles(x + 0.5, y, z + 0.5, 0, 0);
	 * this.blockId = id; this.meta = meta;
	 * 
	 * this.parent = null; }
	 * 
	 * public EntityBlock(World par1World, int x, int y, int z, int id, int
	 * meta, EntityBlockConsole console) { super(par1World);
	 * 
	 * setSize(1.0F, 1.0F); entityCollisionReduction = 1; this.yOffset =
	 * (this.height / 2); this.setLocationAndAngles(x + 0.5, y, z + 0.5, 0, 0);
	 * this.blockId = id; this.meta = meta;
	 ...
	 * 
	 * @Override public void onUpdate() { this.setSpeedFromParent();
	 * super.onUpdate();
	 * 
	 ...
	 * 
	 * this.blockId = nbttagcompound.getInteger("blockId"); this.meta =
	 ...
	 * return false; }
	 * 
	 * public void setSpeedFromParent() { double[] speeds =
	 * this.parent.getSpeed();
	 * 
	 * this.motionX = speeds[0]; this.motionY = speeds[1]; this.motionZ =
	 * speeds[2]; }
	 * 
	 * public void rejoinWorld() { worldObj.setBlock((int) Math.round(posX -
	 * 0.5), (int) Math.round(posY - 0.5), (int) Math.round(posZ - 0.5),
	 * this.blockId, this.meta, 2);
	 * 
	 * this.setDead(); }
	 * 
	 * public AxisAlignedBB getCollisionBox(Entity par1Entity) { if(this.isDead)
	 * { return null; }
	 * 
	 * return par1Entity.boundingBox; }
	 * 
	 * public AxisAlignedBB getBoundingBox() { if(this.isDead) { return null; }
	 * 
